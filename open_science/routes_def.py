...
def like():
    ...
    if action == 'up':      like.is_up = True
    elif action == 'down':  like.is_up = False
    else:                   abort(400)
...
def search_papers_page():
    ...
    papers = search_helper.get_papers_basic_search(search_like, search_option, order,page_num, rows_per_page)
   
    if not papers:
        flash('No results found!', category='warning')
        return redirect(url_for('home_page'))

    #TODO: #Posiuu Query IDs, First_names, Second_names for each author in each paper
    #for paper in papers.items:
    #
  
    return render_template("search/search_paper_result_list.html",papers=papers)
...
def advanced_search_page():
    ...
    return render_template('/search/advanced_search.html',paper_form=paper_form,user_form=user_form,tag_form=tag_form)
...
def advanced_search_papers_page(page,search_data,order_by):
    ...
    return render_template('search/advanced_search_paper.html',page=page, papers=papers, search_data=search_data,order_by=order_by)
...
def advanced_search_users_page(page,search_data,order_by):
    ...
    return render_template('search/advanced_search_user.html',page=page, users=users, search_data=search_data,order_by=order_by)
...
def contact_staff_page():
    ...
    if form.validate_on_submit():
        mssg = MessageToStaff(sender = current_user.id, topic = form.topic.data, text = form.text.data, date = dt.datetime.utcnow() )
        db.session.add(mssg)
        db.session.commit()
