...
def send_profile_delete(email):
    ...
    delete_profile_url = url_for('confirm_profile_delete', token=token, _external=True)
    html = render_template('email/delete_profile_email.html', delete_profile_url=delete_profile_url)
...
def get_emails_cout_last_days(sender_id, email_type_id, days):
    ...
    count = EmailLog.query.filter(EmailLog.email_type_id==email_type_id,func.DATE(EmailLog.date)>=date_after,EmailLog.sender_id==sender_id).count()
    return count

def get_emails_count_to_address_last_days(reciever_email, email_type_id, days):

    date_after = dt.datetime.utcnow().date() - dt.timedelta(days = days)
    count = EmailLog.query.filter(EmailLog.email_type_id==email_type_id,func.DATE(EmailLog.date)>=date_after,EmailLog.receiver_email==reciever_email).count()
    return count
