...
from open_science.tokens import generate_password_confirmation_token, generate_account_recovery_token, generate_email_change_token
...
def send_account_recovery(email):
    ...
    set_password_url = url_for('set_password_page', token=token, _external=True)
    html = render_template('email/reset_password_email.html', set_password_url=set_password_url)
...
def get_emails_cout_last_days(sender_id, email_type, days):
    ...
    count = EmailLog.query.filter(EmailLog.email_type_id==type_id,func.DATE(EmailLog.date)>=date_after,EmailLog.sender_id==sender_id).count()
    return count

def get_emails_count_to_address_last_days(reciever_email,email_type,days):
    ...
    if isinstance(email_type,int):
        ...
        type_id = EmailType.query.filter(EmailType.name==email_type).first().id
    date_after = dt.datetime.utcnow().date() - dt.timedelta(days = days)
    count = EmailLog.query.filter(EmailLog.email_type_id==type_id,func.DATE(EmailLog.date)>=date_after,EmailLog.receiver_email==reciever_email).count()
    return count
